...
namespace NeptuneEvo
{
    ...
    { 
        ...
        [Command("/poster")]
        public static void Poster(Player player)
        {
            player.TriggerEvent("startPoster");
        }
        ...
        public void onResourceStart()
        {
            ...
            {
                // help on spawn
                NAPI.TextLabel.CreateTextLabel("~y~rp.ccrespawn.com", new Vector3(-1028.4111, -2731.3354, 15.545905), 20f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
        ...
        public void Event_entityCreated(Entity entity)
        {
            try
            {
                ...
                if (!vehicle.HasData("ITEMS"))
                {
                    List<nItem> vehItems = new List<nItem>();

                    for (int i = 0; i < 25; i++)
                        vehItems.Add(new nItem(ItemType.Debug, 1));

                    vehicle.SetData("ITEMS", vehItems);
                }
                ...
                else
                {
                    switch (id)
                    {
                        case 0:
                            ...
                            if(Players[player].SpawnPos.DistanceTo2D(new Vector3(1114.848, 234.5277, -50.8409)) < 250.0)
                            {
                                Players[player].SpawnPos = new Vector3(914.08746, 45.85033, 82.09892);
                            }
                    ...
                    case "item_transfer_toplayer":
                        {
                            ...
                            if (tryAdd == -2)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Инвентарь игрока не может вместить больше {nInventory.maxPlayerWeight} кг", 3000);
                                return;
                            }
                    ...
                    case "item_transfer_fromveh":
                        {
                            ...
                            if (Int32.TryParse(text, out transferAmount))
                            {
                                ...
                                if (tryAdd == -2)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Инвентарь не может вместить больше {nInventory.maxPlayerWeight} кг", 3000);
                                    return;
                                }
        ...
        public void ClientEvent_interactionPressed(Player player, params object[] arguments)
        {
            ...
            try
            {
                ...
                if (player.HasData("PT_TABLE"))
                {
                    TexasPoker.ExitPockerTable(player);
                    return;
                }
                ...
                switch (id)
                {
                    ...
                    case 654:
                        ...
                        return;
                    case 655:
                        DiamondCasino.GivePrize(player);
                        return;
                    case 656:
                        DiamondCasino.RollWheel(player);
                        return;
                    case 662:
                    ...
                    case 664:
                        ...
                        return;
                    case 665:
                        Poker.Seat(player, player.GetData<int>("TABLE"), player.GetData<int>("SEAT"));
                        return;
                    case 666:
                        TexasPoker.SeatToTable(player);
                        return;
                    case 1040:
                        CasinoSlots.OpenSlot(player);
                        break;
                    case 1050:
                    ...
                    #endregion
                    case 11111:
                        SquidGames.OpenRegister(player);
                        return;
                    default:
                        return;
                }
                ...
                if (yes)
                {
                    ...
                    switch (callback)
                    {
                        case "CARWASH_PAY":
                            ...
                            return;
                        case "REGISTER_SQUID":
                            SquidGames.RegisterForGame(player);
                            return;
                        case "BUS_RENT":
        ...
        private static void enviromentChangeTrigger()
        {
            try
            {
                SquidGames.CheckRegister();
                SquidGames.GameTimer();
            ...
            try
            {
                ...
                foreach (Player p in Players.Keys.ToList())
                {
                    try
                    {
                        ...
                        if (Main.Players[p].LastSpinWheel > DateTime.Now.AddHours(-4))
                        {
                            TimeSpan dates = DateTime.Now - Main.Players[p].LastSpinWheel;
                            ...
                            Trigger.ClientEvent(p, "showFortune", hourss, Convert.ToInt32(minutess - (hourss * 60)));

                            return;
                        }
                        else
                        {
                            Trigger.ClientEvent(p, "showFortune", 0, 0);
                        }
        ...
        public static void CMD_BUILD(Player client)
        {
            try
            {
                //client.SendChatMessage($"Наш сайт rp.ccRespawn.com");
                //client.SendChatMessage($"Приятной игры!");
            }
            catch { }
        }
        ...
        public static async void CheckDonate(Player player)
        {
            if (player.GetData<DateTime>("DONATE_CHECK") > DateTime.Now) return;

            string login = Accounts[player].Login;

            DataTable result = MySQL.QueryRead($"SELECT * FROM `unitpay_donate` WHERE `login` = '{login}' AND `status` = 1");

            player.SetData("DONATE_CHECK", DateTime.Now.AddMinutes(1));

            if (result == null || result.Rows.Count == 0) return;
            foreach (DataRow Row in result.Rows)
            {
                var id = Convert.ToInt32(Row["id"]);
                var bucks = Convert.ToInt32(Row["redbucks"]);

                Accounts[player].RedBucks += bucks;

                MySQL.Query($"UPDATE `unitpay_donate` SET `status` = '0' WHERE `id` = '{id}'");

                Notify.Send(player, NotifyType.Success, NotifyPosition.TopCenter, $"На ваш счет зачислено {bucks} RB", 3000);
                
            }

            await Accounts[player].Save(player);
        }
    ...
    public static class PasswordRestore
    {
        ...
        private static string mailFrom = config.TryGet<string>("From", "respawnrp@respawn.email");
        ...
        private static string Password = config.TryGet<string>("Pass", "](&vfx`g7x!3/(R{EQO?h4yW");
