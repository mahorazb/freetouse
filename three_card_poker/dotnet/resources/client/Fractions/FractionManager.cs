...
namespace NeptuneEvo.Fractions
{
    ...
    class Stocks : Script
    {
        ...
        public static void Add(int fraction, nItem item, int index)
        {
            List<nItem> items = fracStocks[fraction].Weapons;
            ...
            {
                if (p == null || !Main.Players.ContainsKey(p)) continue;
                if (p.HasData("OPENOUT_TYPE") && p.GetData<int>("OPENOUT_TYPE") == 6 && p.HasData("ONFRACSTOCK") && p.GetData<int>("ONFRACSTOCK") == fraction)
                    Dashboard.OpenOut(p, fracStocks[fraction].Weapons, "Склад оружия", 6);
            }
        }
        ...
        public static void Remove(int fraction, nItem item)
        {
            ...
            {
                items[items.FindIndex(f => f == item)] = new nItem(ItemType.Debug);
            }
            else
            {
