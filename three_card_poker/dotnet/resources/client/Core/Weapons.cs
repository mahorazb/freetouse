    ...
    class Weapons : Script
    {
        ...
        public static void RemoveAll(Player player, bool ammo)
        {
            ...
            for (int i = nInventory.Items[UUID].Count - 1; i >= 0; i--)
            {
                ...
                if (nInventory.WeaponsItems.Contains(nInventory.Items[UUID][i].Type) || nInventory.Items[UUID][i].Type == ItemType.StunGun || (nInventory.Items[UUID][i].Type == ItemType.Mask && !nInventory.Items[UUID][i].IsActive) ||
                    ...
                    nInventory.Items[UUID][i] = new nItem(ItemType.Debug);
            }
            Trigger.ClientEvent(player, "removeAllWeapons");
            NAPI.Task.Run(() => { try { player.RemoveAllWeapons(); } catch { } }, 100);
        ...
        public static void RemoteEvent_changeWeapon(Player player, int key)
        {
            try
            {
                ...
                if (nInventory.ActiveWeapons[UUID][key - 1].Type == ItemType.Debug)
                    return;
                ...
                switch (key)
                {
                    case 1:
                        {
                            if (!nInventory.WeaponsItems.Contains(nInventory.ActiveWeapons[UUID][0].Type) && !nInventory.MeleeWeaponsItems.Contains(nInventory.ActiveWeapons[UUID][0].Type))
                                return;

                            var wItem = nInventory.ActiveWeapons[UUID][0];
                            if (wItem != null) Items.onWeaponUse(player, wItem, 0);
                        }
                        return;
                    case 2:
                        {
                            if (!nInventory.WeaponsItems.Contains(nInventory.ActiveWeapons[UUID][1].Type) && !nInventory.MeleeWeaponsItems.Contains(nInventory.ActiveWeapons[UUID][1].Type))
                                return;

                            var wItem = nInventory.ActiveWeapons[UUID][1];
                            if (wItem != null) Items.onWeaponUse(player, wItem, 1);
                        }
                        return;
                    case 3:
                        {
                            if (!nInventory.WeaponsItems.Contains(nInventory.ActiveWeapons[UUID][2].Type) && !nInventory.MeleeWeaponsItems.Contains(nInventory.ActiveWeapons[UUID][2].Type))
                                return;

                            var wItem = nInventory.ActiveWeapons[UUID][2];
                            if (wItem != null) Items.onWeaponUse(player, wItem, 2);
                        }
                        return;
                }
            }
