...
namespace NeptuneEvo.Core
{
    ...
    class VehicleInventory : Script
    {
        public static void Add(Vehicle vehicle, nItem item)
        {
            ...
            else
            {
                ...
                while (count > 0)
                {
                    ...
                    {
                        int index = FindIndex(vehicle, ItemType.Debug);

                        if (index == -1)
                            break;
                        else
                        {
                            items[index] = new nItem(item.Type, nInventory.ItemsStacks[item.Type], item.Data);
        ...
        public static void Add(Vehicle vehicle, nItem item, int index)
        {
            if (!vehicle.HasData("ITEMS")) return;
            List<nItem> items = vehicle.GetData<List<nItem>>("ITEMS");
            ...
            {
                if (index != -1 && items[index].Type == ItemType.Debug)
                {
                    items[index] = item;
                }
            }
            ...
            {
                if (p == null || !Main.Players.ContainsKey(p)) continue;
                if (p.HasData("OPENOUT_TYPE") && p.GetData<int>("OPENOUT_TYPE") == 2 && p.HasData("SELECTEDVEH") && p.GetData<Vehicle>("SELECTEDVEH") == vehicle)
                {
                    GUI.Dashboard.OpenOut(p, vehicle.GetData<List<nItem>>("ITEMS"), "Багажник", 2);
                }
            }
        }

        public static void Swap(Vehicle vehicle, int indexFrom, int indexTo)
        {
            try
            {
                if (!vehicle.HasData("ITEMS")) return;
                ...
                {
                    nItem temp = (nItem)items[indexTo].Clone();
        ...
        public static int FindIndex(Vehicle vehicle, ItemType type)
        {
        ...
        public static void Split(Vehicle vehicle, int index, int count)
        {
            try
            {
                if (!vehicle.HasData("ITEMS")) return;
                ...
                if (index != -1)
                {
                    int temp = item.Count - count;
                    if (temp > 0)
                    {
                        int toIndex = FindIndex(vehicle, ItemType.Debug);

                        if (toIndex == -1)
                            return;

                        items[toIndex] = new nItem(item.Type, count);
                        //GUI.Dashboard.Update(player, Items[UUID][Items[UUID].Count - 2], Items[UUID].Count - 2);

                        items[index].Count -= count;
                        items[index].Weight = items[index].Count * ItemsWeight.GetItemWeight(item.Type);
                ...
                //Log.Debug($"Item split vehicle :{index.ToString()} ");
                return;
            }
            catch (Exception e)
            {
        ...
        public static void Stack(Vehicle vehicle, int index, int toIndex)
        {
            try
            {
                if (!vehicle.HasData("ITEMS")) return;
                ...
                if ((index != -1 && toIndex != -1) && index != toIndex && items[index].Type == items[toIndex].Type)
                {
                    var fromItem = items[index];
                    var toItem = items[toIndex];

                    int max = (nInventory.ItemsStacks.ContainsKey(toItem.Type)) ? nInventory.ItemsStacks[toItem.Type] : 1;
                    ...
                    items[toIndex].Count = firstCount;
                    items[toIndex].Weight = firstCount * ItemsWeight.GetItemWeight(toItem.Type);

                    //GUI.Dashboard.Update(player, Items[UUID][toIndex], toIndex);


                    if (secondCount > 0)
                    {
                        items[index].Count = secondCount;
                        items[index].Weight = secondCount * ItemsWeight.GetItemWeight(toItem.Type);
                        //GUI.Dashboard.Update(player, Items[UUID][index], index);
                    }
                    else
                    {
                        RemoveAt(vehicle, index, items[index].Count);
        ...
        public static float GetTotalWeight(Vehicle vehicle)
        {
        ...
        public static int TryAdd(Vehicle vehicle, nItem item)
        {
            ...
            {
                if (GetTotalWeight(vehicle) + item.Weight > maxTrunkWeight)//
                    return -2;

                if (FindIndex(vehicle, ItemType.Debug) == -1) return -1;
            }
            else
            {
                if (GetTotalWeight(vehicle) + item.Weight > maxTrunkWeight)
        ...
        public static void RemoveAt(Vehicle vehicle, int index, int count)
        {
            if (!vehicle.HasData("ITEMS")) return;
            List<nItem> items = vehicle.GetData<List<nItem>>("ITEMS");
