...
namespace NeptuneEvo.Core
{
    class BusinessManager : Script
    {
        ...
        public static Dictionary<string, string> RealCarNames = new Dictionary<string, string>();
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    var bank = Convert.ToInt32(Row["money"]);

                    if (!MoneySystem.Bank.Accounts.ContainsKey(bank))
                    {
                        MoneySystem.Bank.CreateWithId(string.Empty, 2, 0, bank);
                    }

                    Business data = new Business(Convert.ToInt32(Row["id"]), Row["owner"].ToString(), Convert.ToInt32(Row["sellprice"]), Convert.ToInt32(Row["type"]), JsonConvert.DeserializeObject<List<Product>>(Row["products"].ToString()), enterpoint, unloadpoint, bank,
                ...
                result = MySQL.QueryRead($"SELECT * FROM realcars");
                if (result == null || result.Rows.Count == 0)
                {
                    Log.Write("DB realcars return null result.", nLog.Type.Warn);
                    return;
                }
                foreach (DataRow Row in result.Rows)
                {
                    CarsNames[5].Add(Row["hash"].ToString());
                    RealCarNames.Add(Row["hash"].ToString(), Row["name"].ToString());
                    ProductsOrderPrice.Add(Row["hash"].ToString(), Convert.ToInt32(Row["price"]));
                    ProductsCapacity.Add(Row["hash"].ToString(), Convert.ToInt32(Row["count"]));
                }

                Log.Write($"Loaded {RealCarNames.Count} real cars", nLog.Type.Success);
        ...
        public static void interactionPressed(Player player)
        {
            ...
            switch (biz.Type)
            {
                ...
                case 12:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны находиться в личном авто", 3000);
                        return;
                    }
                    if (player.Vehicle.Class == 13)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Транспорт не подходит для тюнинга", 3000);
                        return;
                    }
        ...
        public static void RemoteEvent_tuningSeatsCheck(Player player)
        {
            try
            {
                if (!player.IsInVehicle || !player.Vehicle.HasData("ACCESS") || player.Vehicle.GetData<string>("ACCESS") != "PERSONAL")
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Только ваша машина", 3000);
                    return;
                }
                if (player.Vehicle.Class == 13)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Невозможно затюнинговать данный вид транспорта", 3000);
                    return;
                }
                if (player.Vehicle.Class == 8)
                {
                ...
                if (player.HasData("FOLLOWING"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вас кто то держит", 3000);
                    return;
                }
                Business biz = BizList[player.GetData<int>("BIZ_ID")];
        ...
        public static void RemoteEvent_buyTuning(Player player, params object[] arguments)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"У вас нехватает {price - Main.Players[player].Money}$ на покупку данной модификации", 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "В бизнесе закончились продукты", 3000);
                    Trigger.ClientEvent(player, "tunBuySuccess", -2);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы успешно преобрели модификацию", 3000);
