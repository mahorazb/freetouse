    ...
    public class Character : CharacterData
    {
            ...
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        LastSpinWheel = ((DateTime)Row["lastspinwheel"]);
                        Prize = Convert.ToInt32(Row["prize"]);
        ...
        public async Task<bool> Save(Player player)
        {
            try
            {
                ...
                await MySQL.QueryAsync($"UPDATE `characters` SET `personid`='{PersonID}', `pos`='{pos}',`gender`={Gender},`health`={Health},`armor`={Armor},`lvl`={LVL},`exp`={EXP}," +
                    ...
                    $"`licenses`='{JsonConvert.SerializeObject(Licenses)}',`unwarn`='{MySQL.ConvertTime(Unwarn)}',`unmute`='{Unmute}',`lastspinwheel`= '{MySQL.ConvertTime(LastSpinWheel)}', `prize`={ Prize}," +
        ...
        public async Task<int> Create(Player player, string firstName, string lastName)
        {
            try
            {
                ...
                LastSpinWheel = DateTime.Now.AddHours(-4);
                Prize = -1;
