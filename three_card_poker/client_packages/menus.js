...
mp.events.add('openAuto', (models, prices, realnames = []) => {
    if (global.menuCheck()) return;
    autoModels = JSON.parse(models);

    // Подгружаем реальные названия авто
    let autoNames = [];
    if(localplayer.getVariable('CARROOM-DONATE')) {
        autoNames = JSON.parse(realnames);
    }
    else {
...
mp.events.add('client_play_sound', (name) => {
    global.menu.execute(`client_playMusic('https://city-rp.ru/sound/${name}.mp3', 0.2);`);
});
...
mp.events.add('client_slots_bet', (act, data) => {
    switch (act) {
        case 'open':
            if (global.menuCheck()) return;
            global.casinoOpen();
            global.menu.execute(`slots.show('${data}')`);
            break;
        case 'close':

            global.casinoClose();
            global.menu.execute(`slots.hide()`);
            break;
    }
});

mp.events.add('updateSlotsChips', (data) => {
    global.menu.execute(`slots.setChips('${data}')`);
});

mp.events.add('exitSlotsUI', () => {
    mp.events.callRemote('interactionPressed', 1);
});
...
global.poker = mp.browsers.new('package://cef/UI/Poker/index.html');

mp.events.add('casinoPoker', (act, ...data) => {
    switch (act) {
        case 'setChips':
            global.poker.execute(`casinoPoker.setChips('${data[0]}', '${data[1]}')`);
            break;
        case 'setBet':
            global.poker.execute(`casinoPoker.setBet('${data[0]}', '${data[1]}')`);
            break;
        case 'setTime':
            global.poker.execute(`casinoPoker.setTime('${data[0]}', '${data[1]}')`);
            break;
        case 'toggleStart':
            global.poker.execute(`casinoPoker.toggleStart(${data[0]})`);
            break;
        case 'show':
            global.poker.execute(`casinoPoker.show()`);
            break;
        case 'hide':
            global.poker.execute(`casinoPoker.hide()`);
            break;
    }
});