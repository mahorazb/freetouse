...
mp.keys.bind(Keys.VK_1, false, function () { // 1 key
    if (!loggedin || chatActive || cuffed || new Date().getTime() - global.lastCheck < 1000 || global.menuOpened || mp.gui.cursor.visible || localplayer.getVariable('INVISIBLE')) return;
    mp.events.callRemote('changeweap', 1);
    global.lastCheck = new Date().getTime();
});

mp.keys.bind(Keys.VK_2, false, function () { // 2 key
    if (!loggedin || cuffed || chatActive  || new Date().getTime() - global.lastCheck < 1000 || global.menuOpened || mp.gui.cursor.visible || localplayer.getVariable('INVISIBLE')) return;
